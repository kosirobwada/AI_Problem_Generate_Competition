...
def generate_quiz(theme, model="gpt-4", temperature=0):
    try:
        ...
        if retrieved_content:
            prompt = f"以下の情報に基づいて、テーマ:{theme}に関する早押しクイズの問題文とその答えを作成してください。答えは必ず単語で答えられる問題を生成してください。ただし、答えは「答え：」で始めてください。...\n\n情報：{retrieved_content}\n\n問題："
...
def review_quiz(quiz_question, quiz_answer, model="gpt-4", temperature=0.7):
    try:
        prompt = (
