...
def generate_quiz(theme, model="text-davinci-003", temperature=0.7):
    try:
        ...
        if retrieved_content:
            prompt = f"以下の情報に基づいて、テーマ:{theme}に関する早押しクイズの問題文とその答えと解説を作成してください。問題は単語で答えることの可能な問題にしてください。ただし、問題文は、「問題：」で始め、語尾は絶対に「〇〇でしょう？」という形にしてください。また、答えは「答え：」で始めてください。解説は「解説：」で始めてください。...\n\n{retrieved_content}\n\n問題："
            ...
            reference_split = answer_split[1].split("解説：", 1) if len(answer_split) > 1 else [None, None]
