...
def generate_question_answer(theme_content):
    inputs = tokenizer(theme_content, return_tensors="pt")
    with tokenizer.as_target_tokenizer():
        targets = tokenizer("クイズの問題と答えを生成してください", return_tensors="pt").input_ids
    outputs = model.generate(input_ids=inputs["input_ids"], labels=targets)
    return tokenizer.batch_decode(outputs, skip_special_tokens=True)

# クイズ生成
quiz = generate_question_answer(theme_content)
