...
def generate_quiz(theme, model="text-davinci-003", temperature=0.7):
    try:
        ...
        if retrieved_content:
            prompt = f"以下の情報に基づいて、テーマ:{theme}に関する早押しクイズの問題文とその答えと、参照を作成してください。問題は単語で答えることの可能な問題にしてください。ただし、問題文は、「問題：」で始め、語尾は絶対に「〇〇でしょう？」という形にしてください。また、答えは「答え：」で始めてください。参照は「参照：」で始めてください。参照には、問題作成に使用した文書を入れてください。...\n\n{retrieved_content}\n\n問題："
            ...
            if "答え：" in text:
                ...
                return question.strip(), answer.strip(), retrieved_content
            else:
                return "問題が見つかりませんでした", "回答が見つかりませんでした", retrieved_content
        else:
            return "問題が見つかりませんでした", "回答が見つかりませんでした", None
